...
from model.VRDM.VRDM import VRDM
...
@Registers.runners.register_with_name('VRDMRunner')
class VRDMRunner(DiffusionBaseRunner):
    def __init__(self, config):
        super().__init__(config)

    def initialize_model(self, config):
        if config.model.model_type == "VRDM":
            VRDMnet = VRDM(config.model).to(config.training.device[0])
        else:
            raise NotImplementedError
        VRDMnet.apply(weights_init)
        return VRDMnet
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        (x, x_name,x_step), (x_cond, x_cond_name,x_cond_step) = batch
        # print("step:",x_step)
        # print("step_cond:",x_cond_step)
        # self.config.training.n_steps = int(step_x_cond.item())

        # self.config.model.BB.params.num_timesteps = int(x_cond_step.item()/100)
        # print("============",self.config.model.BB.params.num_timesteps)
        x = x.to(self.config.training.device[0])
        x_cond = x_cond.to(self.config.training.device[0])
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        (x, x_name,x_step), (x_cond, x_cond_name, x_cond_step) = batch
        # (x, x_name,step), (x_cond, x_cond_name, step_cond) = batch

        # self.config.training.n_steps = int(step_cond.item())
        # self.config.model.BB.params.num_timesteps = int(x_cond_step.item()/100)
        # print("==============here",self.config.model.BB.params.num_timesteps)
        ...
        #
        # sample = samples[-1]
        sample = net.sample(x_cond,step = int(x_cond_step.item()), clip_denoised=self.config.testing.clip_denoised).to('cpu')
    ...
    def sample_to_eval(self, net, test_loader, sample_path):
        ...
        batch_size = self.config.data.test.batch_size#8
        to_normal = self.config.data.dataset_config.to_normal#True
        sample_num = self.config.testing.sample_num#5
        # index = 0
        for test_batch in pbar:
            # (x_cond, x_cond_name),(x, x_name) = test_batch
            (x_cond, x_cond_name,step_cond),(x, x_name, step) = test_batch

            # self.config.training.n_steps = int(step.item())
            # self.config.model.BB.params.num_timesteps = int(step.item()/100)
            ...
            # x_cond = x_cond[0].squeeze(0)
            # plt.imshow(x_cond,cmap="gray")
            # plt.show()
            ...
            for j in range(sample_num):
                sample = net.sample(x, step = int(step.item()),clip_denoised=False)#The x is frog picture.
                # plt.imshow(sample.squeeze(1).squeeze(0).cpu(),cmap="gray")
                # plt.savefig(f"{index}.png")
                # index = index+1
                # sample = net.sample_vqgan(x)
                # print(range(batch_size+1))
